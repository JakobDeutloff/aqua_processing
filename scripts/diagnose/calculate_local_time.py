...
dataset = xr.concat([ds_first_month, ds_last_two_months], dim="time").astype(float).chunk({'time': 'auto'})

# %% calculate local time
clat = dataset.clat.load()
ds = dataset.where((clat < 20) & (clat > -20), drop=True)
time_local= ds.time.dt.hour + (ds.clon / 15)
time_local = time_local.where(time_local < 24, time_local - 24)
time_local = time_local.where(time_local > 0, time_local + 24)
# %% save iwp 
with ProgressBar():
    time_local.to_netcdf(f'/work/bm1183/m301049/icon_hcap_data/{names[run]}/production/{run}_local_time.nc')
