...
ozone_binned = ozone_control.groupby_bins(
...
def interpolate_pressure(p, height):
    return np.interp(p_grid, p[~np.isnan(p)], height[~np.isnan(p)], left=np.nan, right=np.nan)
...
height_array = height_array.assign_coords(pfull=p_grid, clat_bins=ozone_binned['clat_bins']).compute()
ozone_binned_regrid = ozone_binned.interp(height=height_array, method="linear").compute()
...
ozone_average = xr.Dataset(
    {
        "O3": (["time", "cell", "plev"], o3_gridded[None, :, :]),
    },
    coords=bc_ozone.coords,
    attrs=bc_ozone.attrs,
)
ozone_average['clon_bnds'] = bc_ozone['clon_bnds']
ozone_average['clat_bnds'] = bc_ozone['clat_bnds']
# %% compare tropical mean ozone from control to bc
tropical_mean_control = ozone_average['O3'].where(
    (ozone_average['clat'] > -20) & (ozone_average['clat'] < 20)
).mean('cell')
tropical_mean_bc = bc_ozone['O3'].where(
    (bc_ozone['clat'] > np.deg2rad(-20)) & (bc_ozone['clat'] < np.deg2rad(20))
).mean('cell')

# %%
clat_bin_mids = np.array([interval.mid for interval in ozone_binned['clat_bins'].values])
