...
for run in runs[1:]:
    print(
        f"Increase in diurnal cycle for {line_labels[run]} compared to control by {(((diffs[run]-diffs['jed0011']) / diffs['jed0011'])*100):.0f}%"
    )
...
SW_in_xr = xr.Dataset(
    {
        "SW_in": xr.DataArray(
            SW_in.values,
            coords={"time_points": (bins_gradient[1:] + bins_gradient[:-1]) / 2},
            dims=["time_points"],
        )
    }
)
...
for run in runs:
    ...
    print(
        f"Mean incoming SW for {line_labels[run]}: {incoming_sw[run].values:.2f} W m^-2"
    )
...
for run in runs:
    ...
    print(
        f"Mean incoming SW from hist for {line_labels[run]}: {incoming_sw_hist[run].values:.2f} W m^-2"
    )

# %% diurnal cycle of liquid clouds
rsdt_liq = {}
for run in runs:
    rsdt_liq[run] = (
        datasets[run]["rsdt"]
        .where(
            (datasets[run]["iwp"] > 0.1)
            & (datasets[run]["iwp"] < 1)
            & (datasets[run]["lwp"] > 1e-2)
            & (datasets[run]["conn"] == 0)
        )
        .mean()
    )
